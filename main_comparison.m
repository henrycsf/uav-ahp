...
[~, ~, Network_Metrics_AHP2(:,k), AHP_time2(:,k)] = drone_positioning_AHP(CaseSelect, Pt2(k), N_users2(k), X2, ...
    Y2, h2, DataRate2, Users2{1,k}, Demand2{1,k}, Zcov2(k), Zcap2(k), Rcov2(k), Rcap2(k), nmi, alpha2, beta2, plots);

% [~,~, Network_Metrics_PSO2(:,k), PSO_time2(:,k)] = PSO(CaseSelect, Pt2(k), N_users2(k), X2, Y2, h2, DataRate2, ...
%     Users2{1,k}, Demand2{1,k}, Zcov2(k), Zcap2(k), Rcov2(k), Rcap2(k), nmi, Tol, pop_size, alpha2, beta2, Network_Metrics_AHP2(:,k), plots);

% [~, ~, Network_Metrics_CS2(:,k), CS_time2(:,k)] = cs_positioning(CaseSelect, Pt2(k), N_users2(k), X2, Y2, h2, DataRate2, ...
%     Users2{1,k}, Demand2{1,k}, Zcov2(k), Zcap2(k), Rcov2(k), Rcap2(k), nmi, Tol, pop_size, alpha2, beta2, Network_Metrics_AHP2(:,k), plots);

[~, ~, Network_Metrics_NSGA2(:,k), NSGA_time2(:,k)] = nsga2_positioning(CaseSelect, Pt2(k), N_users2(k), X2, Y2, h2, DataRate2, ...
    Users2{1,k}, Demand2{1,k}, Zcov2(k), Zcap2(k), Rcov2(k), Rcap2(k), nmi, Tol, pop_size, alpha2, beta2, Network_Metrics_AHP2(:,k), plots);

AHP_time(k) = AHP_time2(1,k);
% CS_time(k) = CS_time2(1,k);
% PSO_time(k) = PSO_time2(1,k);
NSGA_time(k) = NSGA_time2(1,k);

Assoc_AHP(k) = Network_Metrics_AHP2(4,k);
% Assoc_CS(k) = Network_Metrics_CS2(4,k);
% Assoc_PSO(k) = Network_Metrics_PSO2(4,k);
Assoc_NSGA(k) = Network_Metrics_NSGA2(4,k);

SINR_AHP(k) = Network_Metrics_AHP2(1,k);
% SINR_CS(k) = Network_Metrics_CS2(1,k);
% SINR_PSO(k) = Network_Metrics_PSO2(1,k);
SINR_NSGA(k) = Network_Metrics_NSGA2(1,k);
...
plot(N_users2, NSGA_time, 'r','LineWidth',2.0,'LineStyle','--')
ylabel('Simulation Time (sec.)','Color','black')
yscale("log")

legend(["$Z_{lim}$","$P_{t}$","Cell Radius $R_{lim}$","Running Time (UAV-AHP)","Running Time (NSGA-II)"],'FontSize',12,'Interpreter','latex','Location', ...
...
plot(N_users2, SINR_NSGA,'LineWidth',1.5,'LineStyle','--')
xlabel("Number of Users (UEs)")
ylabel('Average SINR (dB)')

yyaxis right
plot(N_users2, 1-(Assoc_AHP./N_users2),'LineWidth',1.5)
plot(N_users2, 1-(Assoc_NSGA./N_users2),'LineWidth',1.5,'LineStyle','--')
ylabel('User Outage')

legend(["SINR (UAV-AHP)","SINR (NSGA-II)","Outage (UAV-AHP)","Outage (NSGA-II)"],'FontSize',12,'Interpreter','latex')
